# RTCPeerConnection.RTCPeerConnection.configuration_iceServers_parameter API 兼容性数据
...
```javascript
// 检查RTCPeerConnection.RTCPeerConnection.configuration_iceServers_parameter是否支持
...
if (isRTCPeerConnectionRTCPeerConnectionSupported()) {
    console.log('RTCPeerConnection.RTCPeerConnection.configuration_iceServers_parameter 支持');
    // 使用RTCPeerConnection.RTCPeerConnection.configuration_iceServers_parameter
} else {
    console.log('RTCPeerConnection.RTCPeerConnection.configuration_iceServers_parameter 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCPeerConnection.RTCPeerConnection.configuration_iceServers_parameter polyfill
if (!rtcpeerconnection.RTCPeerConnection) {
    // 在这里添加polyfill实现
    console.log('加载RTCPeerConnection.RTCPeerConnection.configuration_iceServers_parameter polyfill');
}
```
