# CredentialsContainer.create.publicKey_option.extensions API 兼容性数据
...
```javascript
// 检查CredentialsContainer.create.publicKey_option.extensions是否支持
...
if (isCredentialsContainerCreateSupported()) {
    console.log('CredentialsContainer.create.publicKey_option.extensions 支持');
    // 使用CredentialsContainer.create.publicKey_option.extensions
} else {
    console.log('CredentialsContainer.create.publicKey_option.extensions 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// CredentialsContainer.create.publicKey_option.extensions polyfill
if (!credentialscontainer.create) {
    // 在这里添加polyfill实现
    console.log('加载CredentialsContainer.create.publicKey_option.extensions polyfill');
}
```
