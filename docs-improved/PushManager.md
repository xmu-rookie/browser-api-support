# PushManager API 兼容性数据

## 基本信息

- **API名称**: `PushManager`
- **MDN文档**: [PushManager](https://developer.mozilla.org/docs/Web/API/PushManager)
- **规范文档**: [查看规范](https://w3c.github.io/push-api/#pushmanager-interface)
...
function isPushManagerSupported() {
    return 'PushManager' in window || typeof PushManager !== 'undefined';
}

if (isPushManagerSupported()) {
    console.log('PushManager 支持');
    // 使用PushManager
} else {
    console.log('PushManager 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// PushManager polyfill
if (!window.PushManager) {
    // 在这里添加polyfill实现
    console.log('加载PushManager polyfill');
}
```
