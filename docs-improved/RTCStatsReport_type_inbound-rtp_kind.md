# RTCStatsReport.type_inbound-rtp.kind API 兼容性数据
...
```javascript
// RTCStatsReport.type_inbound-rtp.kind 使用示例
// 请查阅MDN文档了解具体用法
console.log('RTCStatsReport.type_inbound-rtp.kind API');
```
...
```javascript
// 检查RTCStatsReport.type_inbound-rtp.kind是否支持
...
if (isRTCStatsReportType_inbound-rtpSupported()) {
    console.log('RTCStatsReport.type_inbound-rtp.kind 支持');
    // 使用RTCStatsReport.type_inbound-rtp.kind
} else {
    console.log('RTCStatsReport.type_inbound-rtp.kind 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_inbound-rtp.kind polyfill
if (!rtcstatsreport.type_inbound-rtp) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_inbound-rtp.kind polyfill');
}
```
