...
```javascript
// 检查Document.queryCommandEnabled是否支持
function isDocumentQueryCommandEnabledSupported() {
    return 'queryCommandEnabled' in document && typeof document.queryCommandEnabled === 'function';
}

if (isDocumentQueryCommandEnabledSupported()) {
    console.log('Document.queryCommandEnabled 支持');
    // 使用Document.queryCommandEnabled
} else {
    console.log('Document.queryCommandEnabled 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// Document.queryCommandEnabled polyfill
if (!document.queryCommandEnabled) {
    // 在这里添加polyfill实现
    console.log('加载Document.queryCommandEnabled polyfill');
}
```
