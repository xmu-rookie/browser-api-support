# ReadableStreamDefaultReader.releaseLock API 兼容性数据
...
```javascript
// ReadableStreamDefaultReader.releaseLock 使用示例
// 请查阅MDN文档了解具体用法
console.log('ReadableStreamDefaultReader.releaseLock API');
```
...
```javascript
// 检查ReadableStreamDefaultReader.releaseLock是否支持
...
if (isReadableStreamDefaultReaderReleaseLockSupported()) {
    console.log('ReadableStreamDefaultReader.releaseLock 支持');
    // 使用ReadableStreamDefaultReader.releaseLock
} else {
    console.log('ReadableStreamDefaultReader.releaseLock 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// ReadableStreamDefaultReader.releaseLock polyfill
if (!readablestreamdefaultreader.releaseLock) {
    // 在这里添加polyfill实现
    console.log('加载ReadableStreamDefaultReader.releaseLock polyfill');
}
```
