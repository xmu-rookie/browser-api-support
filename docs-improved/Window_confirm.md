...
```javascript
// 检查Window.confirm是否支持
function isWindowConfirmSupported() {
    return 'confirm' in window && typeof window.confirm === 'function';
}

if (isWindowConfirmSupported()) {
    console.log('Window.confirm 支持');
    // 使用Window.confirm
} else {
    console.log('Window.confirm 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// Window.confirm polyfill
if (!window.confirm) {
    // 在这里添加polyfill实现
    console.log('加载Window.confirm polyfill');
}
```
