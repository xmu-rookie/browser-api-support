# RTCStatsReport.type_data-channel.protocol API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_data-channel.protocol是否支持
...
if (isRTCStatsReportType_data-channelSupported()) {
    console.log('RTCStatsReport.type_data-channel.protocol 支持');
    // 使用RTCStatsReport.type_data-channel.protocol
} else {
    console.log('RTCStatsReport.type_data-channel.protocol 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_data-channel.protocol polyfill
if (!rtcstatsreport.type_data-channel) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_data-channel.protocol polyfill');
}
```
