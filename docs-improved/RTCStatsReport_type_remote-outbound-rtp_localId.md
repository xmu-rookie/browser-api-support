# RTCStatsReport.type_remote-outbound-rtp.localId API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_remote-outbound-rtp.localId是否支持
...
if (isRTCStatsReportType_remote-outbound-rtpSupported()) {
    console.log('RTCStatsReport.type_remote-outbound-rtp.localId 支持');
    // 使用RTCStatsReport.type_remote-outbound-rtp.localId
} else {
    console.log('RTCStatsReport.type_remote-outbound-rtp.localId 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_remote-outbound-rtp.localId polyfill
if (!rtcstatsreport.type_remote-outbound-rtp) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_remote-outbound-rtp.localId polyfill');
}
```
