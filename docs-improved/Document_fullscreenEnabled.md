# Document.fullscreenEnabled API 兼容性数据
...
```javascript
// 检查Document.fullscreenEnabled是否支持
function isDocumentFullscreenEnabledSupported() {
    return 'fullscreenEnabled' in document && typeof document.fullscreenEnabled === 'function';
}

if (isDocumentFullscreenEnabledSupported()) {
    console.log('Document.fullscreenEnabled 支持');
    // 使用Document.fullscreenEnabled
} else {
    console.log('Document.fullscreenEnabled 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// Document.fullscreenEnabled polyfill
if (!document.fullscreenEnabled) {
    // 在这里添加polyfill实现
    console.log('加载Document.fullscreenEnabled polyfill');
}
```
