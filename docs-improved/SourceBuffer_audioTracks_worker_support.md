...
```javascript
// 检查SourceBuffer.audioTracks.worker_support是否支持
...
if (isSourceBufferAudioTracksSupported()) {
    console.log('SourceBuffer.audioTracks.worker_support 支持');
    // 使用SourceBuffer.audioTracks.worker_support
} else {
    console.log('SourceBuffer.audioTracks.worker_support 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// SourceBuffer.audioTracks.worker_support polyfill
if (!sourcebuffer.audioTracks) {
    // 在这里添加polyfill实现
    console.log('加载SourceBuffer.audioTracks.worker_support polyfill');
}
```
