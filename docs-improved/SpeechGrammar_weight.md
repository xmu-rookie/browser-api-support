# SpeechGrammar.weight API 兼容性数据
...
```javascript
// 检查SpeechGrammar.weight是否支持
function isSpeechGrammarWeightSupported() {
    return 'weight' in speechgrammar && typeof speechgrammar.weight === 'function';
}

if (isSpeechGrammarWeightSupported()) {
    console.log('SpeechGrammar.weight 支持');
    // 使用SpeechGrammar.weight
} else {
    console.log('SpeechGrammar.weight 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// SpeechGrammar.weight polyfill
if (!speechgrammar.weight) {
    // 在这里添加polyfill实现
    console.log('加载SpeechGrammar.weight polyfill');
}
```
