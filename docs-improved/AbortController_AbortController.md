# AbortController.AbortController API 兼容性数据
...
```javascript
// 检查AbortController.AbortController是否支持
function isAbortControllerAbortControllerSupported() {
    return 'AbortController' in abortcontroller && typeof abortcontroller.AbortController === 'function';
}

if (isAbortControllerAbortControllerSupported()) {
    console.log('AbortController.AbortController 支持');
    // 使用AbortController.AbortController
} else {
    console.log('AbortController.AbortController 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
if (!abortcontroller.AbortController) {
    // 在这里添加polyfill实现
    console.log('加载AbortController.AbortController polyfill');
}
```
