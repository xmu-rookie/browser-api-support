# RTCRtpSender.getParameters.return_object_property_degradationPreference API 兼容性数据
...
```javascript
// 检查RTCRtpSender.getParameters.return_object_property_degradationPreference是否支持
...
if (isRTCRtpSenderGetParametersSupported()) {
    console.log('RTCRtpSender.getParameters.return_object_property_degradationPreference 支持');
    // 使用RTCRtpSender.getParameters.return_object_property_degradationPreference
} else {
    console.log('RTCRtpSender.getParameters.return_object_property_degradationPreference 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCRtpSender.getParameters.return_object_property_degradationPreference polyfill
if (!rtcrtpsender.getParameters) {
    // 在这里添加polyfill实现
    console.log('加载RTCRtpSender.getParameters.return_object_property_degradationPreference polyfill');
}
```
