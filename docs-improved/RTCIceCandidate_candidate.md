# RTCIceCandidate.candidate API 兼容性数据
...
```javascript
// 检查RTCIceCandidate.candidate是否支持
function isRTCIceCandidateCandidateSupported() {
    return 'candidate' in rtcicecandidate && typeof rtcicecandidate.candidate === 'function';
}

if (isRTCIceCandidateCandidateSupported()) {
    console.log('RTCIceCandidate.candidate 支持');
    // 使用RTCIceCandidate.candidate
} else {
    console.log('RTCIceCandidate.candidate 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCIceCandidate.candidate polyfill
if (!rtcicecandidate.candidate) {
    // 在这里添加polyfill实现
    console.log('加载RTCIceCandidate.candidate polyfill');
}
```
