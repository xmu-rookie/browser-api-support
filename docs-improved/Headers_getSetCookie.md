# Headers.getSetCookie API 兼容性数据
...
```javascript
// 检查Headers.getSetCookie是否支持
function isHeadersGetSetCookieSupported() {
    return 'getSetCookie' in headers && typeof headers.getSetCookie === 'function';
}

if (isHeadersGetSetCookieSupported()) {
    console.log('Headers.getSetCookie 支持');
    // 使用Headers.getSetCookie
} else {
    console.log('Headers.getSetCookie 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// Headers.getSetCookie polyfill
if (!headers.getSetCookie) {
    // 在这里添加polyfill实现
    console.log('加载Headers.getSetCookie polyfill');
}
```
