# SourceBuffer.updateend_event API 兼容性数据
...
```javascript
// 检查SourceBuffer.updateend_event是否支持
...
if (isSourceBufferUpdateend_eventSupported()) {
    console.log('SourceBuffer.updateend_event 支持');
    // 使用SourceBuffer.updateend_event
} else {
    console.log('SourceBuffer.updateend_event 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// SourceBuffer.updateend_event polyfill
if (!sourcebuffer.updateend_event) {
    // 在这里添加polyfill实现
    console.log('加载SourceBuffer.updateend_event polyfill');
}
```
