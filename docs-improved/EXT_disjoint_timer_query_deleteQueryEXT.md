# EXT_disjoint_timer_query.deleteQueryEXT API 兼容性数据
...
```javascript
// 检查EXT_disjoint_timer_query.deleteQueryEXT是否支持
function isEXT_disjoint_timer_queryDeleteQueryEXTSupported() {
    return 'deleteQueryEXT' in ext_disjoint_timer_query && typeof ext_disjoint_timer_query.deleteQueryEXT === 'function';
}

if (isEXT_disjoint_timer_queryDeleteQueryEXTSupported()) {
    console.log('EXT_disjoint_timer_query.deleteQueryEXT 支持');
    // 使用EXT_disjoint_timer_query.deleteQueryEXT
} else {
    console.log('EXT_disjoint_timer_query.deleteQueryEXT 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// EXT_disjoint_timer_query.deleteQueryEXT polyfill
if (!ext_disjoint_timer_query.deleteQueryEXT) {
    // 在这里添加polyfill实现
    console.log('加载EXT_disjoint_timer_query.deleteQueryEXT polyfill');
}
```
