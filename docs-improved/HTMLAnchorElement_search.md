# HTMLAnchorElement.search API 兼容性数据
...
```javascript
// 检查HTMLAnchorElement.search是否支持
function isHTMLAnchorElementSearchSupported() {
    return 'search' in htmlanchorelement && typeof htmlanchorelement.search === 'function';
}

if (isHTMLAnchorElementSearchSupported()) {
    console.log('HTMLAnchorElement.search 支持');
    // 使用HTMLAnchorElement.search
} else {
    console.log('HTMLAnchorElement.search 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// HTMLAnchorElement.search polyfill
if (!htmlanchorelement.search) {
    // 在这里添加polyfill实现
    console.log('加载HTMLAnchorElement.search polyfill');
}
```
