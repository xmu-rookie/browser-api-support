# RTCStatsReport.type_remote-inbound-rtp.packetsLost API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_remote-inbound-rtp.packetsLost是否支持
...
if (isRTCStatsReportType_remote-inbound-rtpSupported()) {
    console.log('RTCStatsReport.type_remote-inbound-rtp.packetsLost 支持');
    // 使用RTCStatsReport.type_remote-inbound-rtp.packetsLost
} else {
    console.log('RTCStatsReport.type_remote-inbound-rtp.packetsLost 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_remote-inbound-rtp.packetsLost polyfill
if (!rtcstatsreport.type_remote-inbound-rtp) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_remote-inbound-rtp.packetsLost polyfill');
}
```
