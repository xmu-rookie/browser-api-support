# Document.querySelectorAll API 兼容性数据
...
```javascript
// 检查Document.querySelectorAll是否支持
function isDocumentQuerySelectorAllSupported() {
    return 'querySelectorAll' in document && typeof document.querySelectorAll === 'function';
}

if (isDocumentQuerySelectorAllSupported()) {
    console.log('Document.querySelectorAll 支持');
    // 使用Document.querySelectorAll
} else {
    console.log('Document.querySelectorAll 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
if (!document.querySelectorAll) {
    // 在这里添加polyfill实现
    console.log('加载Document.querySelectorAll polyfill');
}
```
