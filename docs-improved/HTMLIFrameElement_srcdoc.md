# HTMLIFrameElement.srcdoc API 兼容性数据
...
```javascript
// 检查HTMLIFrameElement.srcdoc是否支持
...
if (isHTMLIFrameElementSrcdocSupported()) {
    console.log('HTMLIFrameElement.srcdoc 支持');
    // 使用HTMLIFrameElement.srcdoc
} else {
    console.log('HTMLIFrameElement.srcdoc 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// HTMLIFrameElement.srcdoc polyfill
if (!htmliframeelement.srcdoc) {
    // 在这里添加polyfill实现
    console.log('加载HTMLIFrameElement.srcdoc polyfill');
}
```
