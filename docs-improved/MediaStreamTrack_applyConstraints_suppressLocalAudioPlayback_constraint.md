# MediaStreamTrack.applyConstraints.suppressLocalAudioPlayback_constraint API 兼容性数据
...
```javascript
// 检查MediaStreamTrack.applyConstraints.suppressLocalAudioPlayback_constraint是否支持
...
if (isMediaStreamTrackApplyConstraintsSupported()) {
    console.log('MediaStreamTrack.applyConstraints.suppressLocalAudioPlayback_constraint 支持');
    // 使用MediaStreamTrack.applyConstraints.suppressLocalAudioPlayback_constraint
} else {
    console.log('MediaStreamTrack.applyConstraints.suppressLocalAudioPlayback_constraint 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// MediaStreamTrack.applyConstraints.suppressLocalAudioPlayback_constraint polyfill
if (!mediastreamtrack.applyConstraints) {
    // 在这里添加polyfill实现
    console.log('加载MediaStreamTrack.applyConstraints.suppressLocalAudioPlayback_constraint polyfill');
}
```
