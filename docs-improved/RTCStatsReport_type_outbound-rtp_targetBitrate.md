# RTCStatsReport.type_outbound-rtp.targetBitrate API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_outbound-rtp.targetBitrate是否支持
...
if (isRTCStatsReportType_outbound-rtpSupported()) {
    console.log('RTCStatsReport.type_outbound-rtp.targetBitrate 支持');
    // 使用RTCStatsReport.type_outbound-rtp.targetBitrate
} else {
    console.log('RTCStatsReport.type_outbound-rtp.targetBitrate 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_outbound-rtp.targetBitrate polyfill
if (!rtcstatsreport.type_outbound-rtp) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_outbound-rtp.targetBitrate polyfill');
}
```
