# SharedWorker.SharedWorker API 兼容性数据
...
```javascript
// 检查SharedWorker.SharedWorker是否支持
...
if (isSharedWorkerSharedWorkerSupported()) {
    console.log('SharedWorker.SharedWorker 支持');
    // 使用SharedWorker.SharedWorker
} else {
    console.log('SharedWorker.SharedWorker 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// SharedWorker.SharedWorker polyfill
if (!sharedworker.SharedWorker) {
    // 在这里添加polyfill实现
    console.log('加载SharedWorker.SharedWorker polyfill');
}
```
