# RTCStatsReport.type_outbound-rtp.packetsSent API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_outbound-rtp.packetsSent是否支持
...
if (isRTCStatsReportType_outbound-rtpSupported()) {
    console.log('RTCStatsReport.type_outbound-rtp.packetsSent 支持');
    // 使用RTCStatsReport.type_outbound-rtp.packetsSent
} else {
    console.log('RTCStatsReport.type_outbound-rtp.packetsSent 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_outbound-rtp.packetsSent polyfill
if (!rtcstatsreport.type_outbound-rtp) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_outbound-rtp.packetsSent polyfill');
}
```
