...
```javascript
// 检查Window.open.relative-multi-screen是否支持
function isWindowOpenSupported() {
    return 'open' in window && typeof window.open === 'function';
}

if (isWindowOpenSupported()) {
    console.log('Window.open.relative-multi-screen 支持');
    // 使用Window.open.relative-multi-screen
} else {
    console.log('Window.open.relative-multi-screen 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// Window.open.relative-multi-screen polyfill
if (!window.open) {
    // 在这里添加polyfill实现
    console.log('加载Window.open.relative-multi-screen polyfill');
}
```
