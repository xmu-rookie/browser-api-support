# PushSubscription.toJSON API 兼容性数据
...
```javascript
// 检查PushSubscription.toJSON是否支持
function isPushSubscriptionToJSONSupported() {
    return 'toJSON' in pushsubscription && typeof pushsubscription.toJSON === 'function';
}

if (isPushSubscriptionToJSONSupported()) {
    console.log('PushSubscription.toJSON 支持');
    // 使用PushSubscription.toJSON
} else {
    console.log('PushSubscription.toJSON 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// PushSubscription.toJSON polyfill
if (!pushsubscription.toJSON) {
    // 在这里添加polyfill实现
    console.log('加载PushSubscription.toJSON polyfill');
}
```
