# RTCStatsReport.type_transport.iceRole API 兼容性数据
...
```javascript
// RTCStatsReport.type_transport.iceRole 使用示例
// 请查阅MDN文档了解具体用法
console.log('RTCStatsReport.type_transport.iceRole API');
```
...
```javascript
// 检查RTCStatsReport.type_transport.iceRole是否支持
...
if (isRTCStatsReportType_transportSupported()) {
    console.log('RTCStatsReport.type_transport.iceRole 支持');
    // 使用RTCStatsReport.type_transport.iceRole
} else {
    console.log('RTCStatsReport.type_transport.iceRole 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_transport.iceRole polyfill
if (!rtcstatsreport.type_transport) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_transport.iceRole polyfill');
}
```
