# RTCStatsReport.type_local-candidate.candidateType API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_local-candidate.candidateType是否支持
...
if (isRTCStatsReportType_local-candidateSupported()) {
    console.log('RTCStatsReport.type_local-candidate.candidateType 支持');
    // 使用RTCStatsReport.type_local-candidate.candidateType
} else {
    console.log('RTCStatsReport.type_local-candidate.candidateType 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_local-candidate.candidateType polyfill
if (!rtcstatsreport.type_local-candidate) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_local-candidate.candidateType polyfill');
}
```
