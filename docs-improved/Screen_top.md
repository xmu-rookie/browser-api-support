# Screen.top API 兼容性数据

## 基本信息

- **API名称**: `Screen.top`
- **MDN文档**: [Screen.top](https://developer.mozilla.org/docs/Web/API/ScreenDetailed/top)
...
if (isScreenTopSupported()) {
    console.log('Screen.top 支持');
    // 使用Screen.top
} else {
    console.log('Screen.top 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
if (!screen.top) {
    // 在这里添加polyfill实现
    console.log('加载Screen.top polyfill');
}
```
