...
```javascript
// 检查Event.currentTarget是否支持
function isEventCurrentTargetSupported() {
    return 'currentTarget' in event && typeof event.currentTarget === 'function';
}

if (isEventCurrentTargetSupported()) {
    console.log('Event.currentTarget 支持');
    // 使用Event.currentTarget
} else {
    console.log('Event.currentTarget 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// Event.currentTarget polyfill
if (!event.currentTarget) {
    // 在这里添加polyfill实现
    console.log('加载Event.currentTarget polyfill');
}
```
