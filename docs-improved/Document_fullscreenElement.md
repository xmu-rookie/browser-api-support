# Document.fullscreenElement API 兼容性数据
...
```javascript
// 检查Document.fullscreenElement是否支持
function isDocumentFullscreenElementSupported() {
    return 'fullscreenElement' in document && typeof document.fullscreenElement === 'function';
}

if (isDocumentFullscreenElementSupported()) {
    console.log('Document.fullscreenElement 支持');
    // 使用Document.fullscreenElement
} else {
    console.log('Document.fullscreenElement 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
if (!document.fullscreenElement) {
    // 在这里添加polyfill实现
    console.log('加载Document.fullscreenElement polyfill');
}
```
