# RTCStatsReport.type_candidate-pair.responsesSent API 兼容性数据
...
```javascript
// RTCStatsReport.type_candidate-pair.responsesSent 使用示例
// 请查阅MDN文档了解具体用法
console.log('RTCStatsReport.type_candidate-pair.responsesSent API');
```
...
```javascript
// 检查RTCStatsReport.type_candidate-pair.responsesSent是否支持
...
if (isRTCStatsReportType_candidate-pairSupported()) {
    console.log('RTCStatsReport.type_candidate-pair.responsesSent 支持');
    // 使用RTCStatsReport.type_candidate-pair.responsesSent
} else {
    console.log('RTCStatsReport.type_candidate-pair.responsesSent 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_candidate-pair.responsesSent polyfill
if (!rtcstatsreport.type_candidate-pair) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_candidate-pair.responsesSent polyfill');
}
```
