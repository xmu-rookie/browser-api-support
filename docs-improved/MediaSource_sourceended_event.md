# MediaSource.sourceended_event API 兼容性数据
...
```javascript
// 检查MediaSource.sourceended_event是否支持
function isMediaSourceSourceended_eventSupported() {
    return 'sourceended_event' in mediasource && typeof mediasource.sourceended_event === 'function';
}

if (isMediaSourceSourceended_eventSupported()) {
    console.log('MediaSource.sourceended_event 支持');
    // 使用MediaSource.sourceended_event
} else {
    console.log('MediaSource.sourceended_event 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// MediaSource.sourceended_event polyfill
if (!mediasource.sourceended_event) {
    // 在这里添加polyfill实现
    console.log('加载MediaSource.sourceended_event polyfill');
}
```
