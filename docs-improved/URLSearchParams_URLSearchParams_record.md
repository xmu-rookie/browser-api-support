# URLSearchParams.URLSearchParams.record API 兼容性数据
...
```javascript
// 检查URLSearchParams.URLSearchParams.record是否支持
...
if (isURLSearchParamsURLSearchParamsSupported()) {
    console.log('URLSearchParams.URLSearchParams.record 支持');
    // 使用URLSearchParams.URLSearchParams.record
} else {
    console.log('URLSearchParams.URLSearchParams.record 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// URLSearchParams.URLSearchParams.record polyfill
if (!urlsearchparams.URLSearchParams) {
    // 在这里添加polyfill实现
    console.log('加载URLSearchParams.URLSearchParams.record polyfill');
}
```
