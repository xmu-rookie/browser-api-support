# PushManager.permissionState API 兼容性数据
...
```javascript
// PushManager.permissionState 使用示例
// 请查阅MDN文档了解具体用法
console.log('PushManager.permissionState API');
```
...
```javascript
// 检查PushManager.permissionState是否支持
function isPushManagerPermissionStateSupported() {
    return 'permissionState' in pushmanager && typeof pushmanager.permissionState === 'function';
}

if (isPushManagerPermissionStateSupported()) {
    console.log('PushManager.permissionState 支持');
    // 使用PushManager.permissionState
} else {
    console.log('PushManager.permissionState 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// PushManager.permissionState polyfill
if (!pushmanager.permissionState) {
    // 在这里添加polyfill实现
    console.log('加载PushManager.permissionState polyfill');
}
```
