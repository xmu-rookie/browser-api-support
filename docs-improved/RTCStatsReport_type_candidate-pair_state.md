# RTCStatsReport.type_candidate-pair.state API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_candidate-pair.state是否支持
...
if (isRTCStatsReportType_candidate-pairSupported()) {
    console.log('RTCStatsReport.type_candidate-pair.state 支持');
    // 使用RTCStatsReport.type_candidate-pair.state
} else {
    console.log('RTCStatsReport.type_candidate-pair.state 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_candidate-pair.state polyfill
if (!rtcstatsreport.type_candidate-pair) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_candidate-pair.state polyfill');
}
```
