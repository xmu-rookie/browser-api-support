# RTCStatsReport.type_media-source.framesPerSecond API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_media-source.framesPerSecond是否支持
...
if (isRTCStatsReportType_media-sourceSupported()) {
    console.log('RTCStatsReport.type_media-source.framesPerSecond 支持');
    // 使用RTCStatsReport.type_media-source.framesPerSecond
} else {
    console.log('RTCStatsReport.type_media-source.framesPerSecond 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_media-source.framesPerSecond polyfill
if (!rtcstatsreport.type_media-source) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_media-source.framesPerSecond polyfill');
}
```
