...
```javascript
// 检查Event.initEvent是否支持
function isEventInitEventSupported() {
    return 'initEvent' in event && typeof event.initEvent === 'function';
}

if (isEventInitEventSupported()) {
    console.log('Event.initEvent 支持');
    // 使用Event.initEvent
} else {
    console.log('Event.initEvent 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
if (!event.initEvent) {
    // 在这里添加polyfill实现
    console.log('加载Event.initEvent polyfill');
}
```
