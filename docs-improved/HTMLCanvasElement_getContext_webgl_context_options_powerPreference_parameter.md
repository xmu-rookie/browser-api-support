# HTMLCanvasElement.getContext.webgl_context.options_powerPreference_parameter API 兼容性数据
...
```javascript
// 检查HTMLCanvasElement.getContext.webgl_context.options_powerPreference_parameter是否支持
...
if (isHTMLCanvasElementGetContextSupported()) {
    console.log('HTMLCanvasElement.getContext.webgl_context.options_powerPreference_parameter 支持');
    // 使用HTMLCanvasElement.getContext.webgl_context.options_powerPreference_parameter
} else {
    console.log('HTMLCanvasElement.getContext.webgl_context.options_powerPreference_parameter 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// HTMLCanvasElement.getContext.webgl_context.options_powerPreference_parameter polyfill
if (!htmlcanvaselement.getContext) {
    // 在这里添加polyfill实现
    console.log('加载HTMLCanvasElement.getContext.webgl_context.options_powerPreference_parameter polyfill');
}
```
