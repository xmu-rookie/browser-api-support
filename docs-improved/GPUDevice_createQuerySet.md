# GPUDevice.createQuerySet API 兼容性数据
...
```javascript
// 检查GPUDevice.createQuerySet是否支持
...
if (isGPUDeviceCreateQuerySetSupported()) {
    console.log('GPUDevice.createQuerySet 支持');
    // 使用GPUDevice.createQuerySet
} else {
    console.log('GPUDevice.createQuerySet 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// GPUDevice.createQuerySet polyfill
if (!gpudevice.createQuerySet) {
    // 在这里添加polyfill实现
    console.log('加载GPUDevice.createQuerySet polyfill');
}
```
