...
```javascript
// 检查Element.copy_event是否支持
function isElementCopy_eventSupported() {
    return 'copy_event' in element && typeof element.copy_event === 'function';
}

if (isElementCopy_eventSupported()) {
    console.log('Element.copy_event 支持');
    // 使用Element.copy_event
} else {
    console.log('Element.copy_event 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// Element.copy_event polyfill
if (!element.copy_event) {
    // 在这里添加polyfill实现
    console.log('加载Element.copy_event polyfill');
}
```
