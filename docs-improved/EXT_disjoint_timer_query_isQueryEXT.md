# EXT_disjoint_timer_query.isQueryEXT API 兼容性数据
...
```javascript
// 检查EXT_disjoint_timer_query.isQueryEXT是否支持
function isEXT_disjoint_timer_queryIsQueryEXTSupported() {
    return 'isQueryEXT' in ext_disjoint_timer_query && typeof ext_disjoint_timer_query.isQueryEXT === 'function';
}

if (isEXT_disjoint_timer_queryIsQueryEXTSupported()) {
    console.log('EXT_disjoint_timer_query.isQueryEXT 支持');
    // 使用EXT_disjoint_timer_query.isQueryEXT
} else {
    console.log('EXT_disjoint_timer_query.isQueryEXT 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// EXT_disjoint_timer_query.isQueryEXT polyfill
if (!ext_disjoint_timer_query.isQueryEXT) {
    // 在这里添加polyfill实现
    console.log('加载EXT_disjoint_timer_query.isQueryEXT polyfill');
}
```
