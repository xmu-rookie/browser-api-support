# RTCStatsReport.type_inbound-rtp.trackIdentifier API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_inbound-rtp.trackIdentifier是否支持
...
if (isRTCStatsReportType_inbound-rtpSupported()) {
    console.log('RTCStatsReport.type_inbound-rtp.trackIdentifier 支持');
    // 使用RTCStatsReport.type_inbound-rtp.trackIdentifier
} else {
    console.log('RTCStatsReport.type_inbound-rtp.trackIdentifier 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_inbound-rtp.trackIdentifier polyfill
if (!rtcstatsreport.type_inbound-rtp) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_inbound-rtp.trackIdentifier polyfill');
}
```
