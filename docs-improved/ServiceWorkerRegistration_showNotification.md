...
```javascript
// 检查ServiceWorkerRegistration.showNotification是否支持
...
if (isServiceWorkerRegistrationShowNotificationSupported()) {
    console.log('ServiceWorkerRegistration.showNotification 支持');
    // 使用ServiceWorkerRegistration.showNotification
} else {
    console.log('ServiceWorkerRegistration.showNotification 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// ServiceWorkerRegistration.showNotification polyfill
if (!serviceworkerregistration.showNotification) {
    // 在这里添加polyfill实现
    console.log('加载ServiceWorkerRegistration.showNotification polyfill');
}
```
