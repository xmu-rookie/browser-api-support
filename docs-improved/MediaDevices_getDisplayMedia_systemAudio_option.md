# MediaDevices.getDisplayMedia.systemAudio_option API 兼容性数据
...
```javascript
// 检查MediaDevices.getDisplayMedia.systemAudio_option是否支持
...
if (isMediaDevicesGetDisplayMediaSupported()) {
    console.log('MediaDevices.getDisplayMedia.systemAudio_option 支持');
    // 使用MediaDevices.getDisplayMedia.systemAudio_option
} else {
    console.log('MediaDevices.getDisplayMedia.systemAudio_option 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// MediaDevices.getDisplayMedia.systemAudio_option polyfill
if (!mediadevices.getDisplayMedia) {
    // 在这里添加polyfill实现
    console.log('加载MediaDevices.getDisplayMedia.systemAudio_option polyfill');
}
```
