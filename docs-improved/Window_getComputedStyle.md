...
if (isWindowGetComputedStyleSupported()) {
    console.log('Window.getComputedStyle 支持');
    // 使用Window.getComputedStyle
} else {
    console.log('Window.getComputedStyle 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
if (!window.getComputedStyle) {
    // 在这里添加polyfill实现
    console.log('加载Window.getComputedStyle polyfill');
}
```
