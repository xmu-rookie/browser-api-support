# RTCStatsReport.type_transport.iceState API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_transport.iceState是否支持
...
if (isRTCStatsReportType_transportSupported()) {
    console.log('RTCStatsReport.type_transport.iceState 支持');
    // 使用RTCStatsReport.type_transport.iceState
} else {
    console.log('RTCStatsReport.type_transport.iceState 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_transport.iceState polyfill
if (!rtcstatsreport.type_transport) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_transport.iceState polyfill');
}
```
