...
```javascript
// 检查HashChangeEvent是否支持
function isHashChangeEventSupported() {
    return 'HashChangeEvent' in window || typeof HashChangeEvent !== 'undefined';
}

if (isHashChangeEventSupported()) {
    console.log('HashChangeEvent 支持');
    // 使用HashChangeEvent
} else {
    console.log('HashChangeEvent 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// HashChangeEvent polyfill
if (!window.HashChangeEvent) {
    // 在这里添加polyfill实现
    console.log('加载HashChangeEvent polyfill');
}
```
