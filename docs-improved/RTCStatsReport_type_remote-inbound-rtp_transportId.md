# RTCStatsReport.type_remote-inbound-rtp.transportId API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_remote-inbound-rtp.transportId是否支持
...
if (isRTCStatsReportType_remote-inbound-rtpSupported()) {
    console.log('RTCStatsReport.type_remote-inbound-rtp.transportId 支持');
    // 使用RTCStatsReport.type_remote-inbound-rtp.transportId
} else {
    console.log('RTCStatsReport.type_remote-inbound-rtp.transportId 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_remote-inbound-rtp.transportId polyfill
if (!rtcstatsreport.type_remote-inbound-rtp) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_remote-inbound-rtp.transportId polyfill');
}
```
