# USBInTransferResult.worker_support API 兼容性数据
...
```javascript
// 检查USBInTransferResult.worker_support是否支持
function isUSBInTransferResultWorker_supportSupported() {
    return 'worker_support' in usbintransferresult && typeof usbintransferresult.worker_support === 'function';
}

if (isUSBInTransferResultWorker_supportSupported()) {
    console.log('USBInTransferResult.worker_support 支持');
    // 使用USBInTransferResult.worker_support
} else {
    console.log('USBInTransferResult.worker_support 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// USBInTransferResult.worker_support polyfill
if (!usbintransferresult.worker_support) {
    // 在这里添加polyfill实现
    console.log('加载USBInTransferResult.worker_support polyfill');
}
```
