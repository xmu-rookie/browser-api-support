# WorkerGlobalScope.importScripts API 兼容性数据
...
```javascript
// 检查WorkerGlobalScope.importScripts是否支持
...
if (isWorkerGlobalScopeImportScriptsSupported()) {
    console.log('WorkerGlobalScope.importScripts 支持');
    // 使用WorkerGlobalScope.importScripts
} else {
    console.log('WorkerGlobalScope.importScripts 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// WorkerGlobalScope.importScripts polyfill
if (!workerglobalscope.importScripts) {
    // 在这里添加polyfill实现
    console.log('加载WorkerGlobalScope.importScripts polyfill');
}
```
