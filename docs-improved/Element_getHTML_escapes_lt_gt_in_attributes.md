# Element.getHTML.escapes_lt_gt_in_attributes API 兼容性数据
...
```javascript
// 检查Element.getHTML.escapes_lt_gt_in_attributes是否支持
function isElementGetHTMLSupported() {
    return 'getHTML' in element && typeof element.getHTML === 'function';
}

if (isElementGetHTMLSupported()) {
    console.log('Element.getHTML.escapes_lt_gt_in_attributes 支持');
    // 使用Element.getHTML.escapes_lt_gt_in_attributes
} else {
    console.log('Element.getHTML.escapes_lt_gt_in_attributes 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// Element.getHTML.escapes_lt_gt_in_attributes polyfill
if (!element.getHTML) {
    // 在这里添加polyfill实现
    console.log('加载Element.getHTML.escapes_lt_gt_in_attributes polyfill');
}
```
