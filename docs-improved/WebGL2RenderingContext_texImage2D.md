# WebGL2RenderingContext.texImage2D API 兼容性数据
...
```javascript
// 检查WebGL2RenderingContext.texImage2D是否支持
function isWebGL2RenderingContextTexImage2DSupported() {
    return 'texImage2D' in webgl2renderingcontext && typeof webgl2renderingcontext.texImage2D === 'function';
}

if (isWebGL2RenderingContextTexImage2DSupported()) {
    console.log('WebGL2RenderingContext.texImage2D 支持');
    // 使用WebGL2RenderingContext.texImage2D
} else {
    console.log('WebGL2RenderingContext.texImage2D 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// WebGL2RenderingContext.texImage2D polyfill
if (!webgl2renderingcontext.texImage2D) {
    // 在这里添加polyfill实现
    console.log('加载WebGL2RenderingContext.texImage2D polyfill');
}
```
