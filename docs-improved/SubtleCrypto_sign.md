...
if (isSubtleCryptoSignSupported()) {
    console.log('SubtleCrypto.sign 支持');
    // 使用SubtleCrypto.sign
} else {
    console.log('SubtleCrypto.sign 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// SubtleCrypto.sign polyfill
if (!subtlecrypto.sign) {
    // 在这里添加polyfill实现
    console.log('加载SubtleCrypto.sign polyfill');
}
```
