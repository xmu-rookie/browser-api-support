# RTCRtpSender.setParameters.parameters_encodings_parameter.maxFramerate API 兼容性数据
...
```javascript
// 检查RTCRtpSender.setParameters.parameters_encodings_parameter.maxFramerate是否支持
...
if (isRTCRtpSenderSetParametersSupported()) {
    console.log('RTCRtpSender.setParameters.parameters_encodings_parameter.maxFramerate 支持');
    // 使用RTCRtpSender.setParameters.parameters_encodings_parameter.maxFramerate
} else {
    console.log('RTCRtpSender.setParameters.parameters_encodings_parameter.maxFramerate 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCRtpSender.setParameters.parameters_encodings_parameter.maxFramerate polyfill
if (!rtcrtpsender.setParameters) {
    // 在这里添加polyfill实现
    console.log('加载RTCRtpSender.setParameters.parameters_encodings_parameter.maxFramerate polyfill');
}
```
