...
```javascript
// 检查Storage.key是否支持
function isStorageKeySupported() {
    return 'key' in storage && typeof storage.key === 'function';
}

if (isStorageKeySupported()) {
    console.log('Storage.key 支持');
    // 使用Storage.key
} else {
    console.log('Storage.key 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// Storage.key polyfill
if (!storage.key) {
    // 在这里添加polyfill实现
    console.log('加载Storage.key polyfill');
}
```
