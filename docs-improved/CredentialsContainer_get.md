# CredentialsContainer.get API 兼容性数据
...
```javascript
// 检查CredentialsContainer.get是否支持
...
if (isCredentialsContainerGetSupported()) {
    console.log('CredentialsContainer.get 支持');
    // 使用CredentialsContainer.get
} else {
    console.log('CredentialsContainer.get 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// CredentialsContainer.get polyfill
if (!credentialscontainer.get) {
    // 在这里添加polyfill实现
    console.log('加载CredentialsContainer.get polyfill');
}
```
