...
if (isWindowScreenTopSupported()) {
    console.log('Window.screenTop 支持');
    // 使用Window.screenTop
} else {
    console.log('Window.screenTop 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// Window.screenTop polyfill
if (!window.screenTop) {
    // 在这里添加polyfill实现
    console.log('加载Window.screenTop polyfill');
}
```
