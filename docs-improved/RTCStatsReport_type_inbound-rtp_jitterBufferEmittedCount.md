# RTCStatsReport.type_inbound-rtp.jitterBufferEmittedCount API 兼容性数据
...
```javascript
// 检查RTCStatsReport.type_inbound-rtp.jitterBufferEmittedCount是否支持
...
if (isRTCStatsReportType_inbound-rtpSupported()) {
    console.log('RTCStatsReport.type_inbound-rtp.jitterBufferEmittedCount 支持');
    // 使用RTCStatsReport.type_inbound-rtp.jitterBufferEmittedCount
} else {
    console.log('RTCStatsReport.type_inbound-rtp.jitterBufferEmittedCount 不支持，需要polyfill');
    // 加载polyfill或使用替代方案
}
```
...
```javascript
// RTCStatsReport.type_inbound-rtp.jitterBufferEmittedCount polyfill
if (!rtcstatsreport.type_inbound-rtp) {
    // 在这里添加polyfill实现
    console.log('加载RTCStatsReport.type_inbound-rtp.jitterBufferEmittedCount polyfill');
}
```
